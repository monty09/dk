...
classify$run_random_forest_quick <- function(testdf,traindf,variables_to_keep
  ...
  for(i in 1:num_models_to_create){
    rf.Model <- randomForest::randomForest(formula = formula,data = traindf)
...
classify$run_random_forest_tuned <- function(testdf,traindf,variables_to_keep,target_variable
                                             ...
                                             ,num_folds = 5, num_repeats = 5,formula){
  ...
  for(i in 1:num_models_to_create){
    ...
    if(!grid_search){
      ...
      rf_random <- caret::train(formula, data=traindf, method="rf", metric=metric,tuneLength = tuneLength, trControl=control)
      stopCluster(cl)
    }
    ...
    if(grid_search){
      ...
      rf_random <- caret::train(formula, data=traindf, method="rf", metric=metric, tuneLength = tuneLength, trControl=control,tuneGrid = tunegrid)
      stopCluster(cl)    
    }
    ...
    if(accuracy > no_information_rate + percent_higher_than_no_information_rate){
      save(rf_random,file = paste0(directory_to_save_model,"rf_tuned_",i,".Rdata"))
    }
  }
...
classify$run_NaiveBayes_quick <- function(testdf,traindf,formula,variables_to_keep,target_variable
  ...
  for(i in 1:num_models_to_create){
    library(e1071)
    Naive_Bayes_Model= e1071::naiveBayes(formula_all, data=traindf)
    ...
    if(accuracy > no_information_rate + percent_higher_than_no_information_rate){
      save(Naive_Bayes_Model,file = paste0(directory_to_save_model,"naivebayes_quick_",i,".Rdata"))
    }
  }
}
...
classify$run_NaiveBayes_tuned <- function(testdf,traindf,formula,variables_to_keep,target_variable
  ...
  for(i in 1:num_models_to_create){
    ...
    nb.Model <- caret::train(traindf_with_variables, traindf %>% pull(target_variable)
    ...
    if(accuracy > no_information_rate + percent_higher_than_no_information_rate){
      save(nb.Model,file = paste0(directory_to_save_model,"naivebayes_tuned_",i,".Rdata"))
    }
  }
}
...
classify$run_multi_layer_perceptron <- function(testdf,traindf,variables_to_keep,target_variable
  ...
  testdf  <- testdf  %>% dplyr::select(variables_to_keep,target_variable) %>% mutate_at(.funs = funs(as.numeric),.vars = vars(everything())) %>% as.data.frame()
  traindf <- traindf %>% dplyr::select(variables_to_keep,target_variable) %>% mutate_at(.funs = funs(as.numeric),.vars = vars(everything())) %>% as.data.frame()
  traindf_with_variables <- traindf %>% dplyr::select(variables_to_keep)
  ...
  for(i in 1:num_models_to_create){
    ...
    mlp.model <- RSNNS::mlp(x = traindf_with_variables,y = traindf %>% pull(target_variable) %>% as.numeric()
                            ,maxit = iterations,size = hidden_units
    ...
    if(accuracy > no_information_rate + percent_higher_than_no_information_rate){
      save(mlp.model,file = paste0(directory_to_save_model,"mlp_",i,".Rdata"))
    }
  }
}
...
classify$run_svm_quick <- function(testdf,traindf,variables_to_keep,target_variable,formula
  ...
  for(i in 1:num_models_to_create){
    ...
    if(accuracy > no_information_rate + percent_higher_than_no_information_rate){
      save(svm.model,file = paste0(directory_to_save_model,"svm_quick_",i,".Rdata"))
    }
  }
}
...
classify$run_svm_tuned <- function(testdf,traindf,variables_to_keep,target_variable,formula
  ...
  for(i in 1:num_models_to_create){
    ...
    svm.Model <- suppressWarnings(caret::train(form = formula, data=traindf,
                                         method = "svmRadial",
    ...
    if(accuracy > no_information_rate + percent_higher_than_no_information_rate){
      save(svm.Model,file = paste0(directory_to_save_model,"svm_tuned_",i,".Rdata"))
    }
  }
}
